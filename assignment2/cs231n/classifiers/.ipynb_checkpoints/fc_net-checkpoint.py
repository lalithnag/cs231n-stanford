...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W'+str(layer_num+1)] = weight_scale * np.random.randn(hidden_dims[ctr], num_classes)
    ...
    def loss(self, X, y=None):
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        ############################################################################
        dout = [None]*(num_hidden_layers+1)
        
        # Compute softmax loss
        data_loss, dout[num_hidden_layers] = softmax_loss(out[num_hidden_layers], y)
        ...
        # Affine backward layer
        dout[num_hidden_layers-1], grads['W'+str(num_hidden_layers+1)], grads['b'+str(num_hidden_layers+1)] = affine_backward(dout[num_hidden_layers], cache[num_hidden_layers])
        
        for ctr in range(num_hidden_layers-1):
            dout[num_hidden_layers-2-ctr], grads['W'+str(num_hidden_layers-ctr)], grads['b'+str(num_hidden_layers-ctr)] = affine_relu_backward(dout[num_hidden_layers-1-ctr], cache[num_hidden_layers-1-ctr])
        
        dX, grads['W1'], grads['b1'] = affine_relu_backward(dout[0], cache[0])
